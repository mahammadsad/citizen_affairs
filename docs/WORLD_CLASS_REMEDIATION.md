...
| R2 dependency advisory | High build-chain | repository security | Risk owned, audit recorded, upstream-compatible fix adopted when available | verified locally—risk remains | 8 August 2026: `npm audit --omit=dev` reports one high, indirect `nanoid` advisory; no forced lockfile change | Engineering / weekly |
...
## Known security risk: Astro build chain

The audit baseline recorded five high-severity production-tree findings through Astro/Vite/PostCSS/`nanoid`/`vitefu`. A fresh `npm audit --omit=dev` on 8 August 2026 now reports one high-severity indirect `nanoid` advisory (a custom generator can loop when passed size zero). Citizen Affairs does not call that API directly, but the affected package remains in the production dependency tree, so this is tracked as unresolved build/supply-chain risk rather than presented as fixed. Do not use `npm audit fix --force`. Keep the lockfile, pinned GitHub Actions, least-privilege workflows and static output; record fresh evidence at each checkpoint and adopt an upstream-compatible fix when available.
